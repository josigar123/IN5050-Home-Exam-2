...
static void write_SOF0(struct c63_common *cm, struct frame *frame)
{
...
static void write_block(struct c63_common *cm, struct frame *frame, int16_t *in_data, uint32_t width,
                        ...
                        int32_t cc, int channel)
{
  ...
  /* Write motion vector */
  // Read macroblocks from separately passed frame to avoid race-condition when encoding
  struct macroblock *mb =
      &frame->mbs[channel][voffset / 8 * cm->padw[channel] / 8 + uoffset / 8];
...
static void write_interleaved_data_MCU(struct c63_common *cm, struct frame *frame, int16_t *dct,
                                       ...
                                       int16_t *prev_DC, int32_t cc, int channel)
{
  ...
  for (j = y * v * 8; j < (y + 1) * v * 8; j += 8)
  {
    ...
    for (i = x * h * 8; i < (x + 1) * h * 8; i += 8)
    {
      ...
      write_block(cm, frame, dct, wi, he, ii, jj, prev_DC, cc, channel);
    }
  }
}

static void write_interleaved_data(struct c63_common *cm, struct frame *frame)
{
  ...
  for (v = 0; v < vblocks; ++v)
  {
    // Read residuals from separately passed frame to avoid race on encoding when cm->refframe
    for (u = 0; u < ublocks; ++u)
    {
      write_interleaved_data_MCU(cm, frame, frame->residuals->Ydct, cm->ypw,
                                 cm->yph, YX, YY, u, v, &prev_DC[0], yhtbl, 0);
      write_interleaved_data_MCU(cm, frame, frame->residuals->Udct, cm->upw,
                                 cm->uph, UX, UY, u, v, &prev_DC[1], uhtbl, 1);
      write_interleaved_data_MCU(cm, frame, frame->residuals->Vdct, cm->vpw,
...
void write_frame(struct c63_common *cm, struct frame *frame)
{
  ...
  write_SOF0(cm, frame); // Pass separate frame to avoid race condition on keyframe flag
  /* Define Huffman Tables(s) */
  write_DHT(cm);
  /* Start of Scan */
  write_SOS(cm);

  write_interleaved_data(cm, frame); // Pass separate frame to avoid race

  /* End Of Image */
  write_EOI(cm);
  // Write any remaining bits in the output buffer to file
  fwrite(cm->e_ctx.out_buf, 1, cm->e_ctx.out_pos, cm->e_ctx.fp);
  // Reset output buffer position for next frame
  cm->e_ctx.out_pos = 0;
}
