...
__attribute__((always_inline)) static inline uint16_t sad_block_8x8(uint8x8_t orig_rows[8], uint8_t *block2, int stride)
{
    ...
    uint16x8_t acc1 = vabdl_u8(orig_rows[1], vld1_u8(block2 + stride));
    acc0 = vabal_u8(acc0, orig_rows[2], vld1_u8(block2 + 2 * stride));
    acc1 = vabal_u8(acc1, orig_rows[3], vld1_u8(block2 + 3 * stride));
    acc0 = vabal_u8(acc0, orig_rows[4], vld1_u8(block2 + 4 * stride));
    acc1 = vabal_u8(acc1, orig_rows[5], vld1_u8(block2 + 5 * stride));
    acc0 = vabal_u8(acc0, orig_rows[6], vld1_u8(block2 + 6 * stride));
    acc1 = vabal_u8(acc1, orig_rows[7], vld1_u8(block2 + 7 * stride));
